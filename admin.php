...
function updatePatientRecordTable() {
    fetch('P_RECORDS/fetchlatestprecords.php') // Adjust the path as necessary
        .then(response => response.json())
        .then(result => {
            const tableBody = document.getElementById("patientrecordtable").getElementsByTagName('tbody')[0];
            tableBody.innerHTML = ""; // Clear existing table rows

            if (result.length > 0) { // Assuming result is an array of patient records
                result.forEach(patient => {
                    const row = document.createElement('tr');

                    // Populate the row with patient data and action button
                    row.innerHTML = `
                        <td>${patient.p_name}</td>
                        <td>${patient.p_type}</td>
                        <td>
                            <a href="#" class="view-btn" onclick="openPatientRecordModal(${patient.p_id}, '${patient.p_name}')">
                                <img src='VIEW.png' alt='View' style='width: 35px; height: 20px;'>
                            </a>
                        </td>
                    `;
                ...
                const row = document.createElement('tr');
                row.innerHTML = "<td colspan='3'>No patients found</td>";
                tableBody.appendChild(row);
            }
        })
